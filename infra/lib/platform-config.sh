...
load_platform_config() {
  ...
  local -a allowed=(
    ...
    PLATFORM_OBJECT_STORAGE_INTERNAL_NAME PLATFORM_OPERATOR_CIDR PLATFORM_INGRESS_MODE
    PLATFORM_PROVIDER_CIDRS PLATFORM_METADATA_ADDRESS
...
PY
  ...
  # The scoped token is the authoritative provider observation. Avoid project
  # show after it matches the configured UUID: restricted project credentials
  # commonly cannot use the project-list API behind that command.
  [[ ${OS_PROJECT_NAME:-} == "$PLATFORM_PROJECT" ]] || {
