...
def inputs_from_environment(
    ...
    *,
    ...
    return HostUserDataInputs(
        ...
        operator_public_key=_environment_path(values, "OPERATOR_PUBLIC_KEY"),
...
def _public_key(path: Path) -> str:
    key = _decode_text(
        _read_direct(
            path, label="operator public key", maximum_bytes=_MAX_KEY_BYTES, private=False
        ),
        label="operator public key",
    ).strip()
    if "\n" in key or "\r" in key or not key.startswith("ssh-ed25519 "):
        raise ValidationError("operator public key must be one Ed25519 public-key line")
    return key
...
def _replacement_values(
    ...
    key = json.dumps(_public_key(inputs.operator_public_key))
    ...
    if role == "admin":
        return {
            "__OPERATOR_PUBLIC_KEY__": key,
            "__OPERATOR_USER__": OPERATOR_ACCOUNT_NAME,
    ...
    return {
        ...
        "__GARAGE_RPC_PORT__": str(GARAGE_RPC_PORT),
