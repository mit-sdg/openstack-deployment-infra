"""SQLite schema and short, direct controller-state operations."""
...
_NOMAD_JOB_SHA: re.Pattern[str] = re.compile(
...
class DeploymentIdentity:
    """Stable identity cryptographically bound into the database marker."""
...
_IMAGE_ROLE_SQL = ",".join(f"'{role}'" for role in IMAGE_ROLES)
    ...
    Migration(
        ...
        (
            ...
            CREATE TABLE image_selections (
                            role TEXT PRIMARY KEY CHECK (role IN ({_IMAGE_ROLE_SQL})),
...
_GREENFIELD_MARKER_CHECKSUM = hashlib.sha256(DATABASE_GREENFIELD_MARKER).hexdigest()
_DEPLOYMENT_MARKER_PREFIX = DATABASE_DEPLOYMENT_MARKER_PREFIX
...
def validate_complete_schema(
    ...
    """Require the fully migrated, explicitly marked controller schema.
    ...
    if observed_versions != expected_versions:
        raise MigrationError("SQLite migration history is incomplete")
    for row in applied_rows:
