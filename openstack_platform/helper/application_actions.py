...
from ..controller.nomad_jobs import (
...
def inspected_job_identity(value: Mapping[str, Any], job_id: str) -> tuple[int, str, str]:
    ...
    Managed jobs carry the generated-job hash and immutable image explicitly. Jobs
    ...
    marker_identity = (
        metadata.get(NOMAD_CANDIDATE_JOB_SHA_KEY) if isinstance(metadata, dict) else None
    )
    marker_image = metadata.get(NOMAD_CANDIDATE_IMAGE_KEY) if isinstance(metadata, dict) else None
    if (
...
def _inspected_route_marker(
    ...
    *,
    ...
    return uuid(
        metadata.get(NOMAD_ROUTE_MARKER_KEY) if isinstance(metadata, dict) else None,
    ...
    *,
    ...
    def handle(args: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        if (
            ...
            or f"customresponseheaders.{DEPLOYMENT_ROUTE_HEADER}={route_marker}" not in promoted_job
...
def _public_health_from_job(
    ...
    *,
    ...
    metadata = value.get("Meta") if isinstance(value, dict) else None
    raw_marker = metadata.get(NOMAD_ROUTE_MARKER_KEY) if isinstance(metadata, dict) else None
