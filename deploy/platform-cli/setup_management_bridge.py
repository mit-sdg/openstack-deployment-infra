#!/usr/bin/env python3
"""Generate and validate the pinned management SSH/provider bridge."""
...
SSH_ALIAS = "platform-admin"
DEFAULT_PLATFORM_CONFIG = Path("/srv/openstack-platform/config/platform.json")
DEFAULT_SSH_IDENTITY = Path("/srv/openstack-platform/.secrets/ssh/id_ed25519")
DEFAULT_SSH_CONFIG = Path("/srv/openstack-platform/.secrets/ssh/config")
DEFAULT_KNOWN_HOSTS = Path("/srv/openstack-platform/.secrets/ssh/known_hosts")
...
def _platform_identity(path: Path) -> tuple[str, str, str, str]:
    ...
    if (
        ...
        _fail("platform configuration has an invalid management identity")
    try:
...
def _verify_local_dependencies() -> None:
    for command in ("ssh", "ssh-keyscan", "ssh-keygen"):
        ...
        if resolved_name is None:
            _fail(f"management bridge dependency is unavailable: {command}")
        ...
        if (
            ...
            _fail(f"management bridge dependency is not a protected executable: {command}")
...
def preflight(args: argparse.Namespace) -> None:
    if os.geteuid() == 0 or os.environ.get("SUDO_USER"):
        _fail("run management bridge setup as the unprivileged /srv/openstack-platform owner")
    for path, label in (
    ...
    print("management-bridge=prerequisites-ready")
...
def configure(args: argparse.Namespace) -> None:
    ...
    print("management-bridge=verified")


def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="Generate and validate the pinned management SSH/provider bridge"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        print(f"management bridge setup failed: {error}", file=sys.stderr)
        return 1
    except Exception:
        print("management bridge setup failed unexpectedly", file=sys.stderr)
        return 1
