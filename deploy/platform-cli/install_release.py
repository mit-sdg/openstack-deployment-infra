#!/usr/bin/env python3
"""Install one immutable platform CLI release without elevated privileges."""
...
_DEFAULT_OPENSTACK_COMMAND = Path("/srv/openstack-platform/bin/platform-openstack")
...
_DEFAULTS = {
    "management": _Layout(
        release_root=Path("/srv/openstack-platform/platform-cli"),
        bin_root=Path("/srv/openstack-platform/bin"),
        state_root=Path("/srv/openstack-platform/state"),
    ),
}
...
def _sync_management_environment(*, source: Path, release: Path, python: Path, uv: Path) -> Path:
    if not (source / "pyproject.toml").is_file() or not (source / "uv.lock").is_file():
        _fail("management release is missing pyproject.toml or uv.lock")
...
def _launcher(
    ...
    *,
    ...
    module = "platform_cli.cli" if mode == "management" else "platform_cli.helper.main"
    executable = '"$release/.venv/bin/python"' if mode == "management" else shlex.quote(str(python))
...
verify_private_directory() {{
  path=$1
  test -d "$path" && test ! -L "$path" || {{ echo "management configuration is unavailable" >&2; exit 78; }}
  test "$(stat -c %u -- "$path")" = "$(id -u)" && test "$(stat -c %a -- "$path")" = 700 || {{
    echo "management configuration ownership or mode is invalid" >&2
    exit 78
  }}
}}
verify_private_file() {{
  path=$1
  test -f "$path" && test ! -L "$path" || {{ echo "management configuration is unavailable" >&2; exit 78; }}
...
for argument in "$@"; do
  if test -n "$previous_fixed_option"; then
    ...
    echo "management launcher refuses a fixed-path override" >&2
    exit 78
  fi
  case "$argument" in
...
if test -n "$previous_fixed_option"; then
  echo "management launcher refuses an incomplete fixed-path option" >&2
  exit 78
fi
"""
...
launcher=$(readlink -f -- "$0")
release=$(dirname "$(dirname "$launcher")")
test -f "$release/.complete" || {{ echo "no accepted management release" >&2; exit 69; }}
export PYTHONDONTWRITEBYTECODE=1
unset PYTHONPATH
for argument in "$@"; do
  case "$argument" in
    --config-root|--config-root=*|--state-root|--state-root=*)
      echo "management configuration launcher refuses a fixed-path override" >&2
      exit 78
      ;;
  esac
done
exec "$release/.venv/bin/python" -P "$release/source/deploy/platform-cli/install_management_config.py" "$@" --config-root {shlex.quote(str(config_root))} --state-root {shlex.quote(str(state_root))}
"""
...
launcher=$(readlink -f -- "$0")
release=$(dirname "$(dirname "$launcher")")
test -f "$release/.complete" || test -f "$release/.candidate" || {{ echo "no accepted management release" >&2; exit 69; }}
state_directory={shlex.quote(str(state_root))}
platform_config={shlex.quote(str(platform_config))}
test -d "$state_directory" && test ! -L "$state_directory" || {{ echo "management state is unavailable" >&2; exit 78; }}
test -f "$platform_config" && test ! -L "$platform_config" || {{ echo "management platform configuration is unavailable" >&2; exit 78; }}
test "$(stat -c %u -- "$state_directory")" = "$(id -u)" && test "$(stat -c %a -- "$state_directory")" = 700 || {{
  echo "management state ownership or mode is invalid" >&2
  exit 78
}}
export PYTHONDONTWRITEBYTECODE=1
export PYTHONPATH="$release/source"
for argument in "$@"; do
  case "$argument" in
    --destination|--destination=*|--platform-config|--platform-config=*)
      echo "management restore launcher refuses a fixed-path override" >&2
      exit 78
      ;;
  esac
done
exec "$release/.venv/bin/python" -P -m platform_cli.restore "$@" \
...
def _smoke(mode: str, release: Path, python: Path) -> None:
    smoke = release / "source/deploy/platform-cli/release_smoke.py"
    ...
    *,
    ...
    if stat.S_ISLNK(path_metadata.st_mode):
        ...
        if not os.access(resolved, os.R_OK):
            _fail(f"{label} symlink target must be readable by agentops")
        read_path = resolved
...
def _verify_management_configuration(config_root: Path, state_root: Path) -> None:
    try:
        root_metadata = config_root.lstat()
    except FileNotFoundError:
        _fail("install current M1 management configuration before installing a release")
    if (
...
def _install_user_units(source: Path, destination: Path) -> None:
    _ensure_directory(destination, 0o700)
    unit_source = source / "deploy/platform-cli/systemd"
    for name in ("openstack-platform-backup.service", "openstack-platform-backup.timer"):
        ...
        _write_text(destination / name, source_path.read_text(encoding="utf-8"), 0o600)
...
def install(args: argparse.Namespace) -> Path:
    ...
    config_root = (requested_config_root or Path("/srv/openstack-platform/config")).absolute()
    openstack_command = cast(Path, args.openstack_command).absolute()
    ...
    if mode == "management":
        ...
        _verify_management_configuration(config_root, state_root)
    ...
    try:
        ...
        if final.exists():
            ...
            runtime = final / ".venv/bin/python" if mode == "management" else python
            ...
            try:
                ...
                runtime = python
                if mode == "management":
                    runtime = _sync_management_environment(
                        source=source,
                        release=stage,
                        python=python,
                ...
                launcher_name = (
                    "openstack-platform" if mode == "management" else "openstack-platform-helper"
                )
                _write_text(
                    ...
                    _launcher(
                        ...
                        config_root=config_root if mode == "management" else None,
                        state_root=state_root if mode == "management" else None,
                        openstack_command=openstack_command if mode == "management" else None,
                    ),
                    0o550,
                )
                if mode == "management":
                    config_installer = source / "deploy/platform-cli/install_management_config.py"
                    if not config_installer.is_file():
                        _fail("management release is missing its configuration installer")
                    _write_text(
                        stage / "bin/openstack-platform-install-config",
                        _config_launcher(config_root, cast(Path, state_root)),
                        0o550,
                    )
                    _write_text(
        ...
        if mode == "management" and cast(bool, args.install_user_units):
            unit_directory = cast(Path | None, args.user_unit_dir) or (
                Path.home() / ".config/systemd/user"
            )
            _install_user_units(final / "source", unit_directory.absolute())
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Install and atomically select a complete platform CLI release."
    )
    parser.add_argument("--mode", choices=("management", "helper"), required=True)
    ...
    parser.add_argument(
        "--openstack-command", type=Path, default=_DEFAULT_OPENSTACK_COMMAND, help=argparse.SUPPRESS
    )
