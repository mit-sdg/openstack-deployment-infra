...
def _load_action_manifest(source: Path) -> tuple[str, ...]:
    path = source / "platform_cli/helper/actions-v1.txt"
    try:
...
def smoke_management(source: Path, launcher: Path, restore_launcher: Path) -> None:
    module = importlib.import_module("platform_cli.cli")
    if not callable(getattr(module, "main", None)):
        raise SmokeFailure("platform_cli.cli:main is not callable")
    ...
    if not launcher.is_file() or stat.S_IMODE(launcher.stat().st_mode) & 0o111 == 0:
        raise SmokeFailure("the candidate management launcher is unavailable")
    if not restore_launcher.is_file() or stat.S_IMODE(restore_launcher.stat().st_mode) & 0o111 == 0:
        raise SmokeFailure("the candidate M1 restore launcher is unavailable")
    if not (source / "platform_cli/restore.py").is_file():
        raise SmokeFailure("the release is missing its M1 restore implementation")
    ...
    with tempfile.TemporaryDirectory(prefix="platform-release-smoke-") as directory:
        ...
        if result.returncode != 0:
            raise SmokeFailure("the installed management command rejected sanitized configuration")
        ...
        if restore.returncode != 0:
            raise SmokeFailure("the installed M1 restore launcher rejected --help")


def smoke_helper(source: Path) -> None:
    expected = _load_action_manifest(source)
    module = importlib.import_module("platform_cli.helper.main")
    if not callable(getattr(module, "main", None)):
        raise SmokeFailure("platform_cli.helper.main:main is not callable")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("mode", choices=("management", "helper"))
    ...
    if args.mode == "management":
        if args.launcher is None or args.restore_launcher is None:
            raise SmokeFailure("the management smoke requires its installed launchers")
        smoke_management(
            source,
