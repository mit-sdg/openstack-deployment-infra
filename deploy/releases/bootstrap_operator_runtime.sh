...
if [[ $(id -u) -eq 0 || -n ${SUDO_USER:-} ]]; then
  echo "run operator runtime bootstrap as the unprivileged /srv/openstack-platform owner" >&2
  exit 77
fi
...
bridge_script=${PLATFORM_BRIDGE_SCRIPT:-$PWD/deploy/releases/setup_operator_bridge.py}
uv_version=0.12.2
...
for dependency in ssh ssh-keyscan ssh-keygen; do
  command -v "$dependency" >/dev/null 2>&1 || {
    echo "operator bridge dependency is unavailable: $dependency" >&2
    exit 78
  }
done
...
if [[ -e $ssh_identity || -L $ssh_identity ]]; then
  ...
  [[ -f $ssh_identity && ! -L $ssh_identity ]] || {
    echo "operator SSH identity must be a direct private file" >&2
    exit 78
  }
  read -r identity_owner identity_mode <<<"$identity_metadata"
  [[ $identity_owner == $(id -u) && $identity_mode == 600 ]] || {
    echo "operator SSH identity ownership or mode is invalid" >&2
    exit 78
  }
else
  umask 077
  ssh-keygen -q -t ed25519 -N '' -f "$ssh_identity" >/dev/null 2>&1 || {
    echo "could not create the operator SSH identity" >&2
    exit 78
  }
...
[[ $age_owner == 0 || $age_owner == $(id -u) ]] || {
  echo "age must be root-owned or owned by the operator user" >&2
  exit 78
}
(( 0$age_mode & 0022 )) && {
...
printf 'operator-runtime=python-%s uv-%s\n' "$python_version" "$uv_version"
