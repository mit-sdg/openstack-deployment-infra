#!/usr/bin/env python3
"""Atomically install non-secret operator configuration without privileges."""
...
# Direct execution sets sys.path to deploy/releases rather than the archive
...
from openstack_platform.config import load_platform, load_policy  # noqa: E402
from openstack_platform.installation import (  # noqa: E402
    OPERATOR_CONFIG,
    OPERATOR_ROOT,
    OPERATOR_STATE,
)
from openstack_platform.validation import ValidationError  # noqa: E402
...
def install(
    ...
    if os.geteuid() == 0 or os.environ.get("SUDO_USER"):
        _fail(f"run the configuration installer as the unprivileged {OPERATOR_ROOT} owner")
    ...
    print("operator-config=installed")
    return 0


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Install the current platform inventory and private operator policy atomically.",
        allow_abbrev=False,
    )
    parser.add_argument("--platform", type=Path, required=True)
    parser.add_argument("--policy", type=Path, required=True)
    parser.add_argument("--config-root", type=Path, default=OPERATOR_CONFIG)
    parser.add_argument("--state-root", type=Path, default=OPERATOR_STATE)
    return parser
