        ...
        ${
          if role == "admin" then
            ''
              ...
              machine.succeed("cp --dereference /etc/${namespace}/secrets/nomad-gossip-key /root/nomad-gossip-key; systemctl stop ${namespace}-controller.path ${namespace}-controller-activate.service; systemctl stop ${namespace}-controller.service; systemctl stop nomad.service; rm /etc/${namespace}/secrets/nomad-gossip-key; ln -s /root/nomad-gossip-key /etc/${namespace}/secrets/nomad-gossip-key")
              ...
              machine.fail("systemctl start nomad.service")
              machine.succeed("chown root:root /etc/${namespace}/secrets/nomad-gossip-key; systemctl start nomad.service; systemctl start ${namespace}-controller.service; systemctl start ${namespace}-controller-readiness.service; systemctl start ${namespace}-controller.path")
