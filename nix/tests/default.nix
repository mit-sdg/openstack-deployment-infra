        ...
        ${
          if role == "admin" then
            ''
              ...
              machine.succeed("runuser -u management-web -- ${pkgs.curl}/bin/curl --fail --silent --unix-socket /run/${namespace}-controller/project.sock 'http://localhost/v1/health' | grep -F '\"status\":\"ok\"'")
              machine.fail("runuser -u management-web -- ${pkgs.curl}/bin/curl --fail --silent --unix-socket /run/${namespace}-controller/project.sock 'http://localhost/v1/admin/applications?limit=1'")
              ...
              machine.succeed("systemctl cat ${namespace}-management-web.service | grep -F 'CONTROLLER_PROJECT_SOCKET=/run/${namespace}-controller/project.sock'")
