"""Direct M1 application build, deployment, environment, and log operations.
...
DEFAULT_WORKER_COMMAND = ("/srv/app-platform/bin/app-platform-worker",)
DEFAULT_REGISTRY_COMMAND = (
    "/srv/app-platform/tools/service-check-venv/bin/python",
    "/srv/app-platform/infra/registry/delete_manifest.py",
)
_BUILDER_REMOTE_COMMAND = "/run/current-system/sw/bin/app-platform-builder-execute"
_DEPLOY_PHASES = {
    "validated",
    "source_acquired",
    "builder_creating",
...
def parse_dotenv(payload: bytes | str, *, maximum_bytes: int = 262_144) -> dict[str, str]:
    """Parse the deliberately strict, non-executable M1 dotenv subset."""
...
def build_and_push(
    *,
    ...
    recipe: Recipe,
    image_name: str,
    create_builder: Callable[..., Any],
    execute_build: Callable[..., bytes | str | BuildExecution],
    delete_builder: Callable[..., Any],
    timeout_seconds: float = 900,
    deadline: float | None = None,
) -> BuildResult:
    """Create one builder, build and push once, and always request deletion.

    The three callables are concrete deployment hooks. ``execute_build`` must
    return the bounded BuildKit metadata file, not human build output. Recipe
    bytes and source paths are data parameters and are never command text.
    """
    ...
    if (
        ...
        or len(image_name) > 255
        or not _IMAGE_NAME.fullmatch(image_name)
    ):
        raise ValidationError("image name must be a lowercase registry repository without a tag")
    registry = image_name.split("/", 1)[0]
    if ":" in registry and not 1 <= int(registry.rsplit(":", 1)[1]) <= 65_535:
        raise ValidationError("image registry port is invalid")
    ...
    try:
        create_kwargs: dict[str, Any] = {
            "timeout_seconds": _deadline_timeout(
                timeout_seconds, deadline, operation="builder creation"
            )
        }
        execute_kwargs: dict[str, Any] = {
            "timeout_seconds": _deadline_timeout(
                timeout_seconds, deadline, operation="builder execution"
            ),
            "metadata_limit": _MAX_BUILD_METADATA,
        }
        if deadline is not None:
            create_kwargs["deadline"] = deadline
            execute_kwargs["deadline"] = deadline
        create_builder(identifier, **create_kwargs)
        ...
        metadata: bytes | str
        if isinstance(execution, BuildExecution):
            metadata = execution.metadata
            build_log = execution.log
            build_log_truncated = execution.log_truncated
        else:
            metadata = execution
        digest = parse_build_metadata(metadata)
    except BaseException as error:
        primary_error = error
    try:
        # Creation may fail after allocating a server. Deletion is intentionally
        # unconditional and deployment hooks must make it idempotent.
        cleanup_kwargs: dict[str, Any] = {
            "timeout_seconds": _deadline_timeout(
                timeout_seconds, deadline, operation="builder cleanup"
            )
        }
        if deadline is not None:
            cleanup_kwargs["deadline"] = deadline
        delete_builder(identifier, **cleanup_kwargs)
    except BaseException as cleanup_error:
        if isinstance(cleanup_error, (KeyboardInterrupt, SystemExit)):
            raise
        if primary_error is not None:
            raise ApplicationError("builder operation and cleanup both failed") from None
        raise ApplicationError("builder cleanup failed") from None
    if primary_error is not None:
        if isinstance(primary_error, (KeyboardInterrupt, SystemExit)):
            raise primary_error
        raise ApplicationError("builder operation failed; cleanup completed") from None
...
def observe_worker(
    ...
    *,
    ...
    project_id: str | None = None,
    command_runner: Callable[..., Any] = run,
    worker_command: Sequence[str] = DEFAULT_WORKER_COMMAND,
) -> WorkerObservation:
    identifier = uuid(application_id, field="application ID")
    app_slug = slug(application_slug)
...
'''
    ...
    source_markers = (
        ...
        else f'    m1_source_commit        = "{checked_source}"\n'
        f'    m1_recipe_sha256        = "{checked_recipe}"\n'
    )
    marker = f'''  meta {{
...
def nomad_candidate_identity(nomad_job: str) -> tuple[str, str]:
    """Return the exact greenfield candidate identity.

    Unmarked jobs are pre-M1 state and are never treated as deployable or
    ...
    if len(markers) != 1:
        raise ValidationError("Nomad job must contain exactly one M1 candidate marker")
    marker = markers[0]
...
def delete_registry_manifest(
    ...
    *,
    ...
    command_runner: Callable[..., Any] = run,
    registry_command: Sequence[str] = DEFAULT_REGISTRY_COMMAND,
) -> bool:
    app_slug = slug(application_slug)
...
def remove_application_resources(
    application_id: str,
    application_slug: str,
    *,
    prefix: str,
    ...
    """Remove and verify all non-durable application runtime resources.

    Nomad, worker, and registry cleanup are all attempted even if an earlier
    cleanup fails. Managed storage is deliberately outside this operation.
    """
    identifier = uuid(application_id, field="application ID")
    app_slug = slug(application_slug)
    failures: list[BaseException] = []
    job_absent = variable_absent = False
    worker_absent = port_absent = False
    manifest_absent = image_digest is None
    try:
        result = remove_application_job(
            app_slug,
            timeout_seconds=timeout_seconds,
            helper_caller=helper_caller,
        )
        job_absent = result.get("jobAbsent") is True
        variable_absent = result.get("variableAbsent") is True
        if not job_absent or not variable_absent:
            raise ApplicationError("Nomad job or Variable absence was not confirmed")
    except BaseException as error:
        if isinstance(error, (KeyboardInterrupt, SystemExit)):
    ...
    try:
        ...
        worker_absent = worker.server_id is None
        port_absent = worker.port_id is None
    except BaseException as error:
        failures.append(error)
    if image_digest is not None:
        try:
            manifest_absent = delete_registry_manifest(
                app_slug,
                image_digest,
                timeout_seconds=timeout_seconds,
                command_runner=command_runner,
    ...
    if failures:
        interrupt = next(
            (error for error in failures if isinstance(error, (KeyboardInterrupt, SystemExit))),
            None,
        )
        if interrupt is not None:
            raise interrupt
        raise ApplicationError(
            "application cleanup was incomplete; details were withheld"
        ) from None
    return RemovalResult(job_absent, variable_absent, worker_absent, port_absent, manifest_absent)


def remove_application_job(
    application_slug: str,
    *,
    timeout_seconds: float,
    helper_caller: Callable[..., Mapping[str, Any]] = call_helper,
