"""Offline, integrity-checked replacement of the M1 management database."""
...
_AGE_CANDIDATES = (
    Path("/srv/openstack-platform/bin/age"),
    Path("/usr/bin/age"),
    Path("/bin/age"),
)
...
def _materialize_backup(
    ...
    *,
    ...
    if not encrypted and metadata.st_size < len(_SQLITE_HEADER):
        _fail("SQLite backup is too small to be an M1 database")
    if not encrypted:
...
def _verify_candidate(path: Path, *, identity: db.DeploymentIdentity | None = None) -> int:
    ...
    try:
        ...
        if schema_row is None:
            _fail("candidate is not an M1 SQLite database")
        try:
...
def _verify_existing_destination(
    ...
    if not os.path.lexists(path):
        return
    _direct_private_file(
        path, label="existing management database", maximum_bytes=_MAX_BACKUP_BYTES
    )
    ...
    if identity is not None:
        try:
            ...
            raise RestoreError(
                "existing management database belongs to a different deployment identity"
            ) from error
    else:
        try:
            ...
            raise RestoreError(
                "existing management database could not be opened offline"
            ) from error
    try:
        try:
            ...
            raise RestoreError(
                "existing management database could not be inspected for unfinished operations"
            ) from error
        if unfinished:
            _fail(
...
def restore_database(
    ...
    *,
    ...
    """Verify an offline M1 backup and atomically replace ``destination``.

    The source and destination are private mode-0600 files below a private
    mode-0700 state directory.  Older known M1 schemas are migrated in a
    ...
    if old_sidecars and not os.path.lexists(destination_path):
        _fail("SQLite WAL/SHM sidecars exist without a management database")
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Verify and atomically restore an offline M1 SQLite backup",
        allow_abbrev=False,
    )
