"""Strict non-secret inventory and private M1 policy loading."""
...
def platform_config_identity(platform: PlatformConfig) -> str:
    """Hash only the stable deployment inventory used by the M1 state marker.
...
def load_policy(path: str | Path, *, require_private: bool = True) -> Policy:
    ...
    cpu_mhz = _positive_int(standard["cpuMHz"], field="standard.cpuMHz")
    if cpu_mhz != 1_000:
        raise ValidationError("standard.cpuMHz must be exactly 1000 for M1")
...
def load(
    ...
    """Load the two M1 configuration files as one immutable record."""
    return Config(
