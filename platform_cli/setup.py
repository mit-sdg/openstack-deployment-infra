...
MANAGEMENT_ROOT = Path("/srv/openstack-platform")
...
def _platform_document(
    ...
    management_default = f"{ssh_connection[0]}/32" if ssh_connection else None
    management_cidr = _prompt(
        values,
        "PLATFORM_MANAGEMENT_CIDR",
        "Management source CIDR",
        default=management_default,
        input_reader=input_reader,
    )
    try:
        ipaddress.ip_network(management_cidr)
    except ValueError as error:
        raise SetupError("PLATFORM_MANAGEMENT_CIDR is malformed") from error
    ...
    base.update(
        {
            ...
            "images": {role: f"{prefix}-nixos-{role}-{image_suffix}" for role in ROLES},
...
def _ensure_age_identity(age_store: Path, path: Path) -> str:
    ...
    if not path.exists():
        ...
        path.chmod(0o600)
    _direct_private_file(path, field="management backup age identity")
...
def _bootstrap_roles(
    ...
    _command(
        (paths.repository / "deploy/platform-cli/bootstrap_management_runtime.sh",),
        environment=runtime_environment,
        cwd=paths.repository,
    )
    runtime_python = MANAGEMENT_ROOT / "runtime/python3.14"
    _command(
        (
            runtime_python,
            paths.repository / "deploy/platform-cli/setup_management_bridge.py",
            "--platform-config",
            paths.platform,
            "--ssh-identity",
            management_key,
    ...
    _command(
        (
            ...
            ssh_config,
            "--",
            f"platform-admin:{remote_root}/secrets/nomad-tokens.env",
            nomad_tokens,
        ),
        environment=child,
    )
    ...
    for source, destination, mode in transfers:
        ...
        _command(
            ("ssh", "-F", ssh_config, "platform-admin", "--", "chmod", mode, destination),
            environment=child,
        )
    ingress = dict(child)
    ...
    _command(
        (
            runtime_python,
            paths.repository / "deploy/platform-cli/install_management_config.py",
            "--platform",
            paths.platform,
            "--policy",
            paths.policy,
    ...
    _command(
        (
            runtime_python,
            paths.repository / "deploy/platform-cli/install_release.py",
            "--mode",
            "management",
            "--source",
            paths.repository,
    ...
    _command(
        (paths.repository / "deploy/platform-cli/deploy_helper_release.sh", commit),
        environment=child,
        cwd=paths.repository,
    )
    launcher = MANAGEMENT_ROOT / "bin/openstack-platform"
...
def _paths(repository: Path, workspace: Path) -> SetupPaths:
    return SetupPaths(
        ...
        bootstrap=MANAGEMENT_ROOT / ".secrets/setup",
        pki=MANAGEMENT_ROOT / ".secrets/setup/pki",
        openstack_environment=MANAGEMENT_ROOT / ".secrets/openstack.env",
        openstack_wrapper=MANAGEMENT_ROOT / "bin/platform-openstack",
        ssh_directory=MANAGEMENT_ROOT / ".secrets/ssh",
    )
...
def _print_plan(output: TextIO) -> None:
    ...
    print(
        "  install the management bridge, releases, helper, image selections, and backups",
        file=output,
    )
...
def run_setup(
    *,
    ...
    _private_directory(MANAGEMENT_ROOT)
